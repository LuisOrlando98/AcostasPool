...
import { deleteCustomerWithRelations } from "@/lib/customers/delete-customer";
...
const CUSTOMERS_LIST_PATH = "/admin/customers";
const ASSIGNMENTS_PATH = "/admin/customers/assignments";
const ROUTES_PATH = "/admin/routes";
...
async function findCustomerProperty(propertyId: string, customerId: string) {
...
export async function deleteCustomer(
  ...
  const outcome = await deleteCustomerWithRelations(prisma, customerId);
  if (!outcome) {
  ...
  revalidatePath(CUSTOMERS_LIST_PATH);
  revalidatePath(INVOICES_PATH);
 ...
 */
function redirectOnFailure(result: ActionResult, formData: FormData): void {
  if (result.ok) {
    return;
  }
