...
export default function AdminBillingTable({
  ...
  const planOptions = useMemo(
    () =>
      [
        ...new Set(
          rows
            .map((row) => row.contractedPlanName)
  ...
  const saveDraft = () => {
    ...
    startTransition(async () => {
      try {
        // Validation problems come back as a result; unexpected failures throw.
  ...
  const getDraftPriceLabel = (value: string) => {
    ...
    return formatCurrency(parsed, locale);
  };
  ...
  const modal = editing ? (
    <>
        ...
        <div className="app-modal-scroll modal-scroll max-h-[90dvh] overflow-y-auto p-5 pr-4 sm:p-6 sm:pr-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h2 id={editorTitleId} className="text-lg font-semibold">
      ...
      <AppModal
        ...
        zIndexClass="z-[2400]"
        layerClassName="overflow-y-auto p-3 sm:p-6"
        backdropClassName="app-modal-backdrop bg-slate-900/68"
        cardClassName="max-w-lg overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl"
        closeOnBackdrop={!isPending}
        closeOnEscape={!isPending}
      >
        <div className="p-5 sm:p-6">
          ...
          <h3 id={confirmTitleId} className="mt-2 text-lg font-semibold text-slate-900">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600">
              ...
              <select
                ...
                className="rounded bg-transparent pr-1 text-xs text-slate-700 outline-none focus-visible:ring-2 focus-visible:ring-sky-400/70"
              >
                    ...
                    <tr key={row.propertyId} className="transition hover:bg-sky-50/40">
                      ...
                      <td className="px-4 py-3.5 text-right font-semibold text-slate-900">
                        {row.servicePrice === null
                          ? t("common.labels.notAvailable")
                          : formatCurrency(row.servicePrice, locale)}
                      </td>
                      <td className="px-4 py-3.5">
                        {row.paymentType
                          ? t(
                      ...
                      <td className="px-4 py-3.5">
                        <p
                          ...
                          title={row.paymentNotes?.trim() || undefined}
                        >
