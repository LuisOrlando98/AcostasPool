...
const tabs = [
  { href: "/admin/customers", key: "overview" as const },
  { href: "/admin/customers/assignments", key: "assignments" as const },
];

export default function CustomersSectionTabs() {
      ...
      <div className="inline-grid w-auto min-w-[16rem] grid-cols-2 rounded-full border border-slate-200 bg-white p-0.5 shadow-sm sm:min-w-0">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {t(`admin.customers.tabs.${tab.key}`)}
            </Link>
          );
        })}
      </div>
