...
import { normalizePaymentMethod } from "@/lib/customers/financial-info";
import {
...
/** Un campo de texto por elemento del semáforo de estado (`condition_pump`...). */
const poolConditionShape = Object.fromEntries(
  POOL_CONDITION_ITEM_KEYS.map((key) => [`condition_${key}`, textField()])
) as Record<`condition_${(typeof POOL_CONDITION_ITEM_KEYS)[number]}`, ReturnType<typeof textField>>;

const propertyEquipmentShape = {
  filterBrand: textField(),
  filterModel: textField(),
  pumpBrand: textField(),
  pumpHorsepower: textField(),
  poolConditionNotes: textField(),
  ...poolConditionShape,
};
...
/** Equipamiento y semáforo de estado de la piscina, listos para `property.update`. */
function attachPropertyEquipment<T extends Record<string, unknown>>(data: T) {
  return {
    ...data,
    poolEquipment: {
      filterBrand: String(data.filterBrand ?? "") || null,
      filterModel: String(data.filterModel ?? "") || null,
      pumpBrand: String(data.pumpBrand ?? "") || null,
      pumpHorsepower: String(data.pumpHorsepower ?? "") || null,
      poolCondition: parsePoolConditionInput(
        (key) => (typeof data[key] === "string" ? (data[key] as string) : null)
      ),
      poolConditionNotes: String(data.poolConditionNotes ?? "") || null,
    },
  };
}
...
export const updatePropertySchema = z
  .object({
    ...propertyShape,
    ...propertyEquipmentShape,
    propertyId: hiddenId(),
    accessLocationNotes: textField(),
  })
  .transform(attachServicePaymentInfo)
  .transform(attachPropertyEquipment);
...
/**
 * Esquemas de las secciones que preview añadió a la ficha: finanzas del
 * cliente, contrato de servicio y membresía (autopago Stripe).
 */

export const updateCustomerFinancialsSchema = z
  .object({
    customerId: hiddenId(),
    contractedServiceTierId: textField(),
    primaryPropertyId: textField(),
    paymentMethod: textField().transform((value, ctx) => {
      if (!value) {
        return null;
      }
      ...
      if (!normalized) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: CUSTOMER_DETAIL_ERRORS.invalidInput,
        });
        return z.NEVER;
      }
      return normalized;
...
export const generateServiceContractSchema = z.object({ customerId: hiddenId() });

const SIGNATURE_DATA_URL_PREFIX = "data:image/png";

export const signServiceContractSchema = z.object({
  contractId: hiddenId(),
  customerId: hiddenId(),
  signatureDataUrl: requiredText(CUSTOMER_DETAIL_INLINE_ERRORS.signatureRequired).refine(
    (value) => value.startsWith(SIGNATURE_DATA_URL_PREFIX),
    CUSTOMER_DETAIL_INLINE_ERRORS.signatureRequired
  ),
});

export const cancelMembershipSchema = z.object({
  membershipId: hiddenId(),
  customerId: hiddenId(),
  mode: z
    .string()
    .default("period_end")
    .transform((value) => (value === "immediate" ? "immediate" : "period_end")),
});

export const updateContractedPlanSchema = z.object({
  customerId: hiddenId(),
  contractedServiceTierId: requiredText(CUSTOMER_DETAIL_INLINE_ERRORS.planRequired),
});

export const sendServiceStartSchema = z.object({
  customerId: hiddenId(),
  propertyId: hiddenId(),
  contractId: textField(),
});

export const applyMembershipFeeSchema = z.object({
  membershipId: hiddenId(),
  customerId: hiddenId(),
});
