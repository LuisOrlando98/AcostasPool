...
describe("missing key warning", () => {
  ...
  it("stays silent in production and humanizes the missing key", () => {
    vi.stubEnv("NODE_ENV", "production");
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});

    // En producción nunca se muestra la clave cruda: ver missing-key-fallback.test.ts.
    expect(translate(messages, "missing.prodEnv")).toBe("Prod Env");
