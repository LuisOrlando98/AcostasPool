...
function buildTx() {
  return {
    ...
    // Registros `Restrict` que bloquean el borrado: sin ellos, cero.
    serviceContract: { count: vi.fn().mockResolvedValue(0) },
...
describe("deleteCustomerWithRelations", () => {
  ...
  it.each([
    ...
    "no borra nada cuando el cliente tiene %s",
    async (_label, delegate) => {
      ...
      // Act & Assert
      await expect(deleteCustomerWithRelations(db, CUSTOMER_ID)).rejects.toBeInstanceOf(
        CustomerHasFinancialRecordsError
      );
      expect(tx.job.deleteMany).not.toHaveBeenCalled();
      expect(tx.servicePlan.deleteMany).not.toHaveBeenCalled();
  ...
  it("informa de cuántos registros financieros bloquean el borrado", async () => {
    // Arrange
    tx.customer.findUnique.mockResolvedValue(customerRecord);
    tx.serviceContract.count.mockResolvedValue(2);
    tx.payment.count.mockResolvedValue(5);
    tx.membership.count.mockResolvedValue(1);
    const { db } = buildDb(tx);

    // Act
    const error = await deleteCustomerWithRelations(db, CUSTOMER_ID).catch(
      (caught: unknown) => caught
    );

    // Assert
    expect(error).toBeInstanceOf(CustomerHasFinancialRecordsError);
    expect((error as CustomerHasFinancialRecordsError).counts).toEqual({
      contracts: 2,
      payments: 5,
      memberships: 1,
    });
  ...
  it("comprueba los registros financieros antes de borrar nada", async () => {
    // Arrange
    tx.customer.findUnique.mockResolvedValue(customerRecord);
    const { db } = buildDb(tx);
    ...
    // Assert
    for (const delegate of [tx.serviceContract, tx.payment, tx.membership]) {
      expect(delegate.count).toHaveBeenCalledWith({
        where: { customerId: CUSTOMER_ID },
      });
      expect(delegate.count.mock.invocationCallOrder[0]).toBeLessThan(
