...
describe("EMAIL_TEMPLATE_IDS y EMAIL_TEMPLATE_DEFINITIONS", () => {
  it("define 10 ids unicos y una definicion completa para cada uno", () => {
...
describe("getDefaultEmailTemplatesConfig / getDefaultLocalizedEmailTemplatesConfig", () => {
  ...
  it("usa el copy ES cuando existe y cae al default (mismo texto) cuando no hay traduccion", () => {
    ...
    expect(es.INVOICE_SENT.text).toContain("Adjuntamos tu factura {{invoice_number}} en PDF.");
    expect(en.INVOICE_SENT.text).toContain("Your invoice {{invoice_number}} is attached");
...
describe("buildPremiumEmailTemplateHtml", () => {
  ...
  it("envuelve el cuerpo con cabecera (label + subject escapado), bloque de ayuda y pie legal", () => {
    ...
    expect(html).toContain(">Invoice sent</h2>");
  ...
  it("construye los enlaces legales a partir del origen de APP_URL, anteponiendo https si falta el esquema", () => {
